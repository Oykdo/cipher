...
import { ResonanceHalo } from './ResonanceHalo';
...
export function MessageList({
  ...
  const { amplifyMessage, canAmplify, estimatedWeight, currentRho, minRhoRequired, availableAether } = useSocialInteractions(sessionUserId);
  const [amplifying, setAmplifying] = useState<Set<string>>(new Set());
  ...
  const handleAmplify = async (message: MessageV2) => {
    if (amplifying.has(message.id)) return;
    setAmplifying(prev => new Set(prev).add(message.id));

    // Optimistic UI handled by Halo intensity updates (future)
    // For now, simple interaction
    const success = await amplifyMessage(message.id, message.senderId, message.conversationId);

    if (success) {
      // Could show a toast or local state update
    } else {
      alert(t('messages.amplify_failed', 'Impossible d\'amplifier : Fonds insuffisants ou réputation trop faible.'));
    }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <AnimatePresence>
        {messages
          .filter(message => !message.isBurned)
          .map((message) => {
            ...
            // Placeholder for visual resonance intensity
            // Ideally this comes from message metadata updated via socket
            const resonanceIntensity = (message as any).resonanceScore || 0;
            ...
            if (attachmentData && !burned) {
              return (
                ...
                >
                  <ResonanceHalo intensity={resonanceIntensity}>
                    <div className="relative group">
                      ...
                      />
                      {/* Amplify Button for Received Attachments */}
                      {!isOwn && !burned && (
                        <button
                          onClick={() => canAmplify ? handleAmplify(message) : alert(t('messages.amplify_rho_required', `Résonance insuffisante (${(currentRho * 100).toFixed(0)}% < ${(minRhoRequired * 100).toFixed(0)}% requis)`))}
                          className={`absolute -right-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full ${
                            canAmplify 
                              ? 'text-quantum-cyan hover:bg-quantum-cyan/10' 
                              : 'text-muted-grey cursor-not-allowed'
                          }`}
                          title={canAmplify 
                            ? t('messages.amplify_tooltip', `Lancer un Lovebomb (Coût: ${estimatedWeight.toFixed(1)} Aether)`)
                            : t('messages.amplify_disabled', `Résonance insuffisante (${(currentRho * 100).toFixed(0)}% < ${(minRhoRequired * 100).toFixed(0)}% requis)`)}
                          disabled={amplifying.has(message.id)}
                        >
                          ⚡
                        </button>
                      )}
              ...
              >
                <ResonanceHalo intensity={resonanceIntensity}>
                  <div className="relative group flex items-center">
                    {/* Amplify Button for Received Text Messages (Left side if own? No, only receive) */}
                    {/* Actually for received messages, the bubble is on left. Button should be on right of bubble. */}
                    ...
                    >
                      ...
                      {burned ? (
                        ...
                        <>
                          ...
                          {/* Burn After Reading - Countdown for received messages */}
                          ...
                          {/* Burn After Reading - Simple indicator for sent messages */}
                          ...
                          <div className="flex items-center justify-between mt-2 text-xs text-muted-grey">
                            <div className="flex items-center gap-2">
                              {/* Encryption/P2P badges */}
                              {message.encryptionType && (
                                <span
                                  className={`flex items-center gap-1 px-1.5 py-0.5 rounded ${message.encryptionType.includes('double-ratchet')
                                      ? 'bg-purple-500/20 text-purple-400'
                                      : 'bg-quantum-cyan/20 text-quantum-cyan'
                                    }`}
                                >
                                  {message.encryptionType.includes('double-ratchet') ? '🔐' : '🔒'}
                                </span>
                              )}
                              {message.isP2P && (
                              ...
                              {resonanceIntensity > 0 && (
                                <span className="text-quantum-cyan font-bold flex items-center gap-1">
                                  ⚡ {resonanceIntensity.toFixed(0)}
                                </span>
                              )}
                            </div>
                    ...
                    {/* Amplify Button for Text (Right side) - Always visible on hover */}
                    {!isOwn && !burned && !locked && (
                      <button
                        onClick={() => {
                          if (!canAmplify) {
                            alert(t('messages.amplify_rho_required', `Résonance insuffisante (${(currentRho * 100).toFixed(0)}% < ${(minRhoRequired * 100).toFixed(0)}% requis)`));
                            return;
                          }
                          if (availableAether < estimatedWeight * 0.2) {
                            alert(t('messages.amplify_aether_required', `Aether insuffisant (${availableAether.toFixed(1)} < ${(estimatedWeight * 0.2).toFixed(1)} requis)`));
                            return;
                          }
                          handleAmplify(message);
                        }}
                        className={`ml-2 opacity-0 group-hover:opacity-100 transition-opacity p-2 rounded-full ${
                          canAmplify && availableAether >= estimatedWeight * 0.2
                            ? 'text-quantum-cyan hover:bg-quantum-cyan/10 hover:scale-110' 
                            : 'text-muted-grey/50 cursor-not-allowed'
                        }`}
                        title={
                          !canAmplify 
                            ? t('messages.amplify_disabled', `Résonance insuffisante (${(currentRho * 100).toFixed(0)}% < ${(minRhoRequired * 100).toFixed(0)}% requis)`)
                            : availableAether < estimatedWeight * 0.2
                              ? t('messages.amplify_aether_required', `Aether insuffisant`)
                              : t('messages.amplify_tooltip', `⚡ Lovebomb (Coût: ${(estimatedWeight * 0.2).toFixed(1)} Aether)`)
                        }
                        disabled={amplifying.has(message.id)}
                      >
                        ⚡
